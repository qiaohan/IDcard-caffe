 ...
 */
static image_double ll_angle( image_double* in_img,int channel_num, double threshold,
                              struct coorlist ** list_p, void ** mem_p,
                              image_double * modgrad, unsigned int n_bins )
{
  image_double g;
  unsigned int height,width,x,y,adr,i;
  ...
  /* check parameters */
  if( channel_num < 1 || in_img == NULL || in_img[0]->data == NULL || in_img[0]->xsize == 0 || in_img[0]->ysize == 0 )
  ...
  height = in_img[0]->ysize;
  width = in_img[0]->xsize;

  /* allocate output image */
  g = new_image_double(width,height);

  /* get memory for the image of gradient modulus */
  *modgrad = new_image_double(width,height);

  /* get memory for "ordered" list of pixels */
  list = (struct coorlist *) calloc( (size_t) (height*width), sizeof(struct coorlist) );
  *mem_p = (void *) list;
  ...
  for(x=0;x<width;x++) g->data[(height-1)*width+x] = NOTDEF;
  for(y=0;y<height;y++) g->data[width*y+width-1]   = NOTDEF;

  /* compute gradient on the remaining pixels */

  double* gradient= new double[channel_num];
  double* angleval = new double[channel_num];
  int chosenidx;
  for(x=0;x<width-1;x++){
	  for(y=0;y<height-1;y++){
		  adr = y*width+x;
		  for (int c = 0; c != channel_num ; ++c){
			  com1 = in_img[c]->data[adr+width+1] - in_img[c]->data[adr];
			  com2 = in_img[c]->data[adr+1]   - in_img[c]->data[adr+width];
			  gx = com1+com2; /* gradient x component */
			  gy = com1-com2; /* gradient y component */
			  norm2 = gx*gx+gy*gy;
			  gradient[c] = sqrt( norm2 / 4.0 );
			  angleval[c] = atan2(gx,-gy);
		  }
		  chosenidx = std::max_element(gradient, gradient+3) - gradient;
		  norm = gradient[chosenidx];
...
double * LineSegmentDetection( int * n_out,
	double ** img, int channel_num, int X, int Y,
	...
	int ** reg_img, int * reg_x, int * reg_y )
{
	...
	image = new image_double[channel_num];

	/* load and scale image (if necessary) and compute angle at each pixel */
	for (int i = 0; i != channel_num ; ++i){
		image[i] = new_image_double_ptr( (unsigned int) X, (unsigned int) Y, img[i] );
	}


	if( scale != 1.0 ){
		scaled_image = new image_double[channel_num];
		for (int i = 0; i != channel_num ; ++i){
			scaled_image[i] = gaussian_sampler( image[i], scale, sigma_scale );
		}
		angles = ll_angle( scaled_image, channel_num, rho, &list_p, &mem_p,
			&modgrad, (unsigned int) n_bins );
		for (int i = 0; i != channel_num ; ++i){
			free_image_double(scaled_image[i]);
		}
		delete [] scaled_image;
	}
	else{
		angles = ll_angle( image,channel_num, rho, &list_p, &mem_p, &modgrad,
			(unsigned int) n_bins );
	}
	xsize = angles->xsize;
	ysize = angles->ysize;
                           ...
                           int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  return LineSegmentDetection( n_out, &img, 1, X, Y, scale, sigma_scale, quant,
